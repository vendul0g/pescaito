...
class TLDCountry:
    ...
    def get_country_from_tld(self, similar_domain_name: str):
        """
        ...
        if '.' in similar_domain_name:
            tld = similar_domain_name.split('.')[-1]
        # Obtenemos el país asociado al TLD del fichero JSON
        country = self.tld_by_country.get(tld.lower())
        if country is None:
            print(f"[!] No se encuentra el pais para el TLD dado: {similar_domain_name}")
        return country
